...
export function CostsTab({ data, status }: CostsTabProps) {
    ...
    // Waterfall Data definition
    const waterfallData = [
        { name: "Gross Return", uv: [0, gross], fill: "hsl(var(--primary))" },
        { name: "Fees", uv: [gross - fees, gross], fill: "rgba(245, 158, 11, 0.85)" },
        { name: "Taxes", uv: [gross - fees - taxes, gross - fees], fill: "rgba(239, 68, 68, 0.85)" },
        { name: "Borrow Fees", uv: [gross - fees - taxes - borrow, gross - fees - taxes], fill: "rgba(249, 115, 22, 0.85)" },
        { name: "Margin Interest", uv: [gross - fees - taxes - borrow - margin, gross - fees - taxes - borrow], fill: "rgba(234, 179, 8, 0.85)" },
        { name: "Net Return", uv: [0, net], fill: "hsl(var(--chart-1))" }
    ];
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                {largestDrag && largestDrag.value > 0.01 && (
                    <Badge variant="outline" className={`px-3 py-1 text-xs font-semibold self-start sm:self-center ${largestDrag.badgeClass}`}>
                        <ShieldAlert className="w-3.5 h-3.5 mr-1" /> Largest Drag: {largestDrag.label} (-{largestDrag.value.toFixed(2)}%)
                    </Badge>
                )}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Decomposition of Gross Return down to Net Return (%)</CardDescription>
                    </CardHeader>
                                ...
                                <BarChart data={waterfallData} margin={{ top: 10, right: 30, left: 10, bottom: 5 }}>
                                    ...
                                    <Tooltip
                                        ...
                                        formatter={(value: any, name: any, props: any) => {
                                            const range = props.payload.uv;
                                            const diff = Math.abs(range[1] - range[0]);
                                            return [`${diff.toFixed(2)}%`, "Change"];
                                        }}
                                    />
                                    <Bar dataKey="uv">
